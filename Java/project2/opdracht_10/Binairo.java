...
public class Binairo extends JFrame implements ActionListener {
	...
	private String htmlf = "<html><font color=red>";
	private String htmlb = "</font></html>";
	private int wiscount = 0;

	private LinkedHashMap<String, JLabel> labels = new LinkedHashMap<String, JLabel>();

	public Binairo() throws IOException {
...

	{
		try {
			FileReader fr = new FileReader(
					"C:/Users/Tom/workspace/Java/project2/opdracht_10/Puzzel.txt");
			...
			while (true) {
				...
				if (s.equals("1") || s.equals("0")) {
					...
					color = htmlf + s + htmlb; // geeft de vaste 0 en 1 de kleur
												// rood
			...
			for (int i = 0; i < 144; i++) {
				if (lijst.get(i).equals("-")) {
					if (i < 2) {
						...
						wiscount = i;
						//disableWis();
						System.out.println(wiscount);
						break;

					} else if (lijst.get(i - 1).equals("1")) {
						lijst.set(i, "0");
						draw();
						rij();
						kolom();
					...
					} else if (lijst.get(i - 2).equals("1")) {
						lijst.set(i, "0");
						draw();
						rij();
						kolom();
	...
	public void colorCheck() {
		for (int i = 0; i < 144; i++) {
			...
			if (lijst.get(i).equals(color)) {
				wiscount = wiscount - 2;

			} else if (lijst.get(i) != color) {
				lijst.set(wiscount, "-");
				wiscount--;
			}
